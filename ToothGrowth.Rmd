...
# group outcomes by dose and compare delivery method
...
```{r boxplots, fig.height = 6, fig.width = 10, echo = FALSE}
...
```{r show_summary_stats, echo = FALSE}
group_stats
```
...
permutation_diffs_1 <- # difference in group means for permuted delivery labels
            sapply(1:10000, function(i) test_diff(dose_1$len, 
                                                  sample(dose_1$supp)))
# p-value is computed below using proportion of differences whose absolute value
# exceeds the observed difference. Use absolute value since test is two sided.
p_val_dose_1 <- mean(abs(permutation_diffs_1) > abs(observed_diff_1))
...
permutation_diffs_2 <- # difference in group means for permuted delivery labels
            sapply(1:10000, function(i) test_diff(dose_2$len, 
                                                  sample(dose_2$supp)))
p_val_dose_2 <- mean(abs(permutation_diffs_2) > abs(observed_diff_2))

# adjusted p-values due to multiple hypothesis tests
adj_p_vals <- p.adjust(c(t_test_0.5$p.value,
...
- 0.5 mg/day Dose Level t-test
  - p-value = `r round(t_test_0.5$p.value,3)`
  - adjusted p-value = `r round(adj_p_vals[1],3)`
- 1 mg/day Dose Level permutation test
  - p-value = `r round(p_val_dose_1,3)`
  - adjusted p-value = `r round(adj_p_vals[2],3)`
- 2 mg/day Dose Level permutation test
  - p-value = `r round(p_val_dose_2,3)`
  - adjusted p-value = `r round(adj_p_vals[3],3)`  
...
```{r boxplots_appendix, eval = FALSE}
treatment_boxplots # tooth length vs delivery method box plots
```


```{r summary_stats_appendix, eval = FALSE}
group_stats <- ToothGrowth %>% 
               group_by(as.factor(dose),supp) %>% 
               summarise(median = median(len), mean = mean(len), sd = sd(len))
```

```{r show_summary_stats_appendix, eval = FALSE}
group_stats
```
...
```{r hyp_tests_appendix, eval = FALSE}
# t-test on dose level 0.5 mg/day
OJ_0.5 <- ToothGrowth %>% filter(supp == 'OJ' & dose == 0.5) # OJ @ 0.5 mg dose
VC_0.5 <- ToothGrowth %>% filter(supp == 'VC' & dose == 0.5) # VC @ 0.5 mg dose
t_test_0.5 <- t.test(OJ_0.5$len,VC_0.5$len, alternative = 'two.sided') # t-test
